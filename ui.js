...
// ---- NAV ----
var NAV_MAP = { home:0, messenger:1, 'user-chat':2, 'group-chat':3, browse:4, 'persona-mgr':5, 'create-char':6, settings:7, admin:8, updates:9 };
...
function nav(id) {
    ...
    if (id === 'user-chat')   renderUserChat(false);   // false = don't re-open DM
    if (id === 'group-chat')  renderGroupChat(false);  // false = don't re-init history
...
function renderPersonaList() {
    ...
    CAI.getUserPersonas(function(err, personas) {
        ...
        personas.forEach(function(p) {
            ...
            (function(pid) { useBtn.onclick = function() { CAI.activePersonaId = pid; renderPersonaList(); }; })(p.id);
...
function loadSettingsPage() {
    ...
    var s = CAI.ollamaSettings.load();
    var mdEl = document.getElementById('s-ollama-model');
    if (mdEl) mdEl.value = s.model;
    ...
    // AI Settings visible to ALL users (WebLLM runs client-side)
    var aiBlock = document.getElementById('ai-settings-block');
    if (aiBlock) aiBlock.style.display = 'block';
    var aiStatusBtn = document.getElementById('ai-settings-btn');
    if (aiStatusBtn) aiStatusBtn.style.display = 'inline-block';
}
function saveSettingsPage() {
    var sel = document.getElementById('s-ollama-model');
    var mdl = sel ? sel.value : '';
    if (!mdl) { alert('Please select a model.'); return; }
    CAI.ollamaSettings.save(mdl);
...
function fetchModelsFromSettings() {
    // No-op for WebLLM; models are static
}
function testConnectionFromSettings() {
    var sel = document.getElementById('s-ollama-model');
    var mdl = sel ? sel.value : '';
    var res = document.getElementById('s-test-result');
    if (res) { clearEl(res); res.style.color='#664400'; res.appendChild(document.createTextNode('Checking...')); }
    CAI.testOllama(mdl, function(err, msg) {
        if (!res) return;
        clearEl(res);
...
function loadWebLLMFromSettings() {
    var sel  = document.getElementById('s-ollama-model');
    var mdl  = sel ? sel.value : '';
    var hint = document.getElementById('s-models-hint');
    if (!mdl) { alert('Select a model first.'); return; }
    if (hint) { clearEl(hint); hint.appendChild(document.createTextNode('Loading model... this may take a while.')); }
    CAI.loadWebLLMModel(mdl, function(text, progress) {
        if (hint) { clearEl(hint); hint.appendChild(document.createTextNode(text || 'Loading...')); }
    }, function(err, msg) {
        if (hint) {
            clearEl(hint);
            if (err) { hint.style.color='#CC0000'; hint.appendChild(document.createTextNode('Error: '+err)); }
            else     { hint.style.color='#006600'; hint.appendChild(document.createTextNode(msg||'Ready!')); }
        }
    });
}
...
function renderUserChat(reopen) {
    ...
    CAI._ucSidebarTimer = setInterval(function() {
        if (document.getElementById('user-chat') &&
            document.getElementById('user-chat').classList.contains('active')) {
            _rebuildUCSidebar();
        } else {
    ...
    CAI.getOnlineUsers(function(err, online) {
        ...
        msgs.forEach(function(m, idx) {
            ...
            if (isMe && idx === msgs.length - 1) {
                // Determine status
                var seenByOther = m.read === true;  // server marks our msg read when other opens chat
                var otherOnline = online && online.indexOf(other) >= 0;
                var status = seenByOther ? 'seen' : (otherOnline ? 'delivered' : 'sent');
...
function sendGCUserMsg() {
    ...
    CAI.getUserPersonas(function(err, personas) {
        ...
        for (var i=0;i<personas.length;i++) if (personas[i].id===CAI.activePersonaId){user=personas[i];break;}
        if (!user && personas.length) user=personas[0];
        CAI.gcPost('users', CAI.currentUser, text, 'user', function() { loadGCHistory('users'); });
    });
}
...
// Shared GC bot caller
// Shared GC bot caller — uses WebLLM engine
function callGCBot(bot, messages, kind, cb) {
    var model = CAI.ollamaSettings.load().model;
    CAI.getWebLLMEngine(model, function(text, progress) {
        ...
        engine.chat.completions.create({
            ...
            if (resp && resp.choices && resp.choices[0] && resp.choices[0].message && resp.choices[0].message.content) {
                reply = resp.choices[0].message.content.replace(/^\s+|\s+$/g, '');
...
function gcOpenAddUser(kind) {
    ...
    // Only mutual followers can be invited
    CAI.getMutualFollowers(function(mutual) {
        if (!mutual.length) {
            list.innerHTML='<div style="padding:10px;color:#888;font-style:italic;font-size:11px;">No mutual follows yet. Both users must follow each other to be invitable.</div>';
        } else {
            mutual.forEach(function(u) {
...
function gcPickerConfirm() {
    ...
    if (CAI._gcPickerMode === 'user') {
        // Invite all chosen users, then refresh the member list
        var pending = chosen.length;
        chosen.forEach(function(u) {
            CAI.gcInvite(kind, u, function() {
                pending--;
                if (pending === 0) {
                    if (kind === 'mixed') renderGCMixedUsersList();
                    if (kind === 'users') loadGCHistory('users');
                }
            });
        });
    } else {
